"""
Run FMPE inference on a held-out ABC test spectrum and produce a corner plot.

Loads the fm4ar checkpoint from configs/fmpe_abc/model__best.pt, samples
10,000 posterior samples for one test planet, and overlays the ground-truth
nested sampling posterior from Level2Data/Ground Truth Package/Tracedata.hdf5.

Usage (run from Project/):
    python scripts/infer_fmpe_abc.py [--planet-idx N]

--planet-idx: index into abc_test.hdf (default 0). Change to see different planets.
"""
...
CKPT_DIR = Path("configs/fmpe_abc")
...
def main(planet_idx: int = 0) -> None:
    ...
    # Load fm4ar checkpoint + theta scaler (samples come out in scaled space)
    ckpt_path = CKPT_DIR / "model__best.pt"
    print(f"Loading FMPE checkpoint from {ckpt_path} ...")
    model = build_model(
        file_path=ckpt_path,
    ...
    # sample_batch uses context["flux"].shape[0] as the number of samples,
    # so we replicate the single spectrum N_SAMPLES times in the batch dim.
    print(f"Sampling {N_SAMPLES} posterior samples ...")
    context = {
        "flux": flux.unsqueeze(0).expand(N_SAMPLES, -1).to(device),   # (N_SAMPLES, 52)
        "wlen": wlen.unsqueeze(0).expand(N_SAMPLES, -1).to(device),   # (N_SAMPLES, 52)
    }

    with torch.no_grad():
        samples_scaled = model.sample_batch(context=context).cpu().numpy()  # (N_SAMPLES, 6)

    # Inverse-transform from normalised space back to physical units
    samples_fmpe = theta_scaler.inverse_array(samples_scaled)
    print(f"  Samples shape: {samples_fmpe.shape}")
    ...
    with h5py.File(TRACEDATA, "r") as f:
        ...
        if key in f:
            td = f[key]["tracedata"][:]    # (N_ns, 6)
            wt = f[key]["weights"][:]      # (N_ns,)
            # IS-weighted resample to get unweighted samples
    ...
    legend_items = [
        plt.Line2D([0], [0], color="#033696", lw=2, label="FMPE (this model)"),
    ]
    if has_gt:
        legend_items.append(
    ...
    out_path = OUT_DIR / f"abc_fmpe_planet{planet_id}.png"
